        ...
        if body then
            ...
            if type(decoded) == "table" and type(decoded.tree) == "table" then
                for _, entry in ipairs(decoded.tree) do
                    ...
                    if entry.type == "tree" and (path == "installer" or path:sub(-10) == "/installer") then
                        config.branch = branch
                        config.root = path == "installer" and "" or path:sub(1, #path - 10)
                        return entry.sha
                    end
                end
                last_err = "InstallerFolderNotInRepo"
            else
